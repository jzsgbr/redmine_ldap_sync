...
module LdapSync::EntityManager
  ...
  private
    def get_user_fields(username, user_data=nil, options={})
      ...
      user_fields = (user_data.respond_to?(:to_h) ? user_data.to_h : user_data).inject({}) do |fields, (attr, value)|
    ...
    def get_group_fields(groupname, group_data = nil)
      ...
      group_fields = (group_data.respond_to?(:to_h) ? group_data.to_h : group_data).inject({}) do |fields, (attr, value)|
    ...
    def ldap_users
      ...
      with_ldap_connection do |ldap|
        ...
        users_on_ldap = changes.values.reduce(Set.new, :+).map(&:downcase)
    ...
    def find_user(ldap, login, attrs, &block)
      ...
      # Ha a login nem tartalmaz @-t, keresünk sAMAccountName-mel is
      # (UPN: user@domain.com, sAMAccountName: user)
      login_filter = if login !~ /@/
        Net::LDAP::Filter.eq( setting.login, login ) |
        Net::LDAP::Filter.eq( :samaccountname, login )
      else
        Net::LDAP::Filter.eq( setting.login, login )
      end
