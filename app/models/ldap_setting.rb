...
# Scheduler explicit betöltése - a to_prepare nem fut le időben
...
class LdapSetting
  # Rails 7: ez a fájl az eager load során töltődik be, mielőtt az infectors/user.rb
  # definiálná a User::STANDARD_FIELDS konstanst. Ezért itt definiáljuk fallback-ként.
  ...
  SCHEDULE_FIELDS = %w( schedule_enabled schedule_frequency schedule_hour schedule_minute schedule_days schedule_next_run_at schedule_last_run_at )
  SCHEDULE_FREQUENCIES = %w( every_15_min every_30_min hourly every_2h every_4h every_6h every_12h daily weekly )
  ...
  safe_attributes *(LDAP_ATTRIBUTES + CLASS_NAMES + FLAGS + COMBOS + OTHERS + SCHEDULE_FIELDS)
  define_attribute_methods LDAP_ATTRIBUTES + CLASS_NAMES + FLAGS + COMBOS + OTHERS + SCHEDULE_FIELDS
  ...
  def schedule_enabled?
    [true, '1', 'yes'].include? @attributes[:schedule_enabled]
  end

  def schedule_frequency
    @attributes[:schedule_frequency] || 'daily'
  end

  def schedule_hour
    (@attributes[:schedule_hour] || 2).to_i
  end

  def schedule_minute
    (@attributes[:schedule_minute] || 0).to_i
  end

  def schedule_days
    days = @attributes[:schedule_days] || []
    days.is_a?(Array) ? days.map(&:to_i) : []
  end

  def schedule_next_run_at
    @attributes[:schedule_next_run_at]
  end

  def schedule_last_run_at
    @attributes[:schedule_last_run_at]
  end

  def update_schedule_timestamps!(last_run, next_run)
    @attributes[:schedule_last_run_at] = last_run.iso8601
    @attributes[:schedule_next_run_at] = next_run&.iso8601
    self.settings = delete_unsafe_attributes(@attributes, User.current)
  end

  def save_schedule(params)
    @attributes[:schedule_enabled]   = params[:schedule_enabled] == '1'
    @attributes[:schedule_frequency] = params[:schedule_frequency]
    @attributes[:schedule_hour]      = params[:schedule_hour].to_i
    @attributes[:schedule_minute]    = params[:schedule_minute].to_i
    @attributes[:schedule_days]      = (params[:schedule_days] || {}).select { |_, v| v == '1' }.keys.map(&:to_i)
    # Mindig újraszámítjuk a next_run_at-et az ÚJ beállítások alapján
    # (frequency váltáskor is frissül)
    if @attributes[:schedule_enabled]
      next_run = LdapSync::Scheduler.next_run_for(self)
      @attributes[:schedule_next_run_at] = next_run&.iso8601
    else
      @attributes[:schedule_next_run_at] = nil
    end
    self.settings = delete_unsafe_attributes(@attributes, User.current)
  end
